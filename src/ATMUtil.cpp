...
namespace ATMUtil {
    ...
    User::Account *login() {
        ...
        auto *const curr_user = User::get_user(User::input("your username"));
        if (!curr_user) {
        ...
        do {
            ...
            pin = stoi(User::input("your PIN (0 to exit)", "int"));
            pinTrials++;
    ...
    int show_main_menu() {
        ...
        const int opNum = stoi(User::input("the operation number you want to make", "int"));
        if (opNum < 0 || opNum > 5) {
    ...
    void check_balance(const User::Account *const curr_user) {
        const std::string string_balance = User::convert_to_thousand_separated(curr_user->balance);
        printf("Your credit is %s EGP\n", string_balance.c_str());
    }

    void withdraw_cash(User::Account * const curr_user) {
        const int amount = stoi(User::input("the amount to withdraw (0 to exist)", "int"));
        if (amount != 0) {
            if (amount > curr_user->balance)
                ...
                printf("Successfully withdrawn %s EGP balance\n", User::convert_to_thousand_separated(amount).c_str());
                curr_user->balance -= amount;
            }
        }
    }

    void deposit_cash(User::Account * const curr_user) {
        const int amount = stoi(User::input("the amount to deposit (0 to exist)", "int"));
        if (amount != 0) {
            if (amount % 50)
    ...
    void transfer_balance(User::Account * const curr_user) {
        const std::string recipient_username = User::input("the username of the recipient (0 to exit)");
        if (recipient_username != "0") {
            auto *const recipient = User::get_user(recipient_username);
            if (!recipient) {
                printf("Invalid username\n");
                return;
            }
            ...
            const int amount = stoi(User::input("the amount to transfer (0 to exist)", "int"));
            if (amount != 0) {
                if (amount > curr_user->balance)
                    printf("Not enough balance to transfer\n");
                else {
                    printf("Successfully transferred %s EGP balance to %s\n",
                           User::convert_to_thousand_separated(amount).c_str(), recipient->username.c_str());
                    curr_user->balance -= amount;
    ...
    bool change_pin(User::Account * const curr_user) {
        int oldPin;
        if (User::change_pin_inputs(oldPin)) {
            if (oldPin != curr_user->pin)
                printf("Wrong old PIN. Exiting ...\n");
            else {
                int newPin, confirmationPin;
                if (User::change_pin_inputs(newPin, "new") && User::change_pin_inputs(confirmationPin, "confirmation")) {
